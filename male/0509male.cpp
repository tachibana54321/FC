...
int main() {
	...
	for(int i=0; i<60; i++) {
		sprintf(file_name,"H:\\n2n0_M\\new60table\\male_outtable%d.%d.txt",i,sequence);
    ...
    for(int f=start; f<=end; f++) {
        ...
        sprintf(file_name,"output.%d.new_20150827.txt",f);
    	...
    	fclose(data);
    	printf("output.%d.new_20150827 fin\n",f);
        fprintf(log,"output.%d.new_20150827 fin\n",f);
    }
	free(data);
    for(int i=0; i<60; i++) {
            fprintf(table[i],"?");
