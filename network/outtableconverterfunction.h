...
void outtable_converter(int amount, int cycle, int start, int end) {
    FILE *input,*output,*all;
    char neuro_array[22835][20];
    char file_name[80];
    char user_input[20];
    char c,*buffer;
    int progress, counter, buffer_index, none_zero_counter[60], index[2];
    double value;
	sprintf(user_input,"%d_result",amount);
	...
	for(int i=0; i<60; i++) none_zero_counter[i]=0;
        ...
        for(int ff=0; ff<cycle; ff++) {
        	...
        	while(true) {
                ...
                if(progress == 2) {
                    ...
                    if( value > 0 ) {
                        //A&B point
                        fprintf(output,"%s %s %.12lf %d\n", neuro_array[index[0]], neuro_array[index[1]], value, f);
                        //A&B zone
