...
void matrix_file2array(int f, int *matrix) {
	...
	int n = 22835*22835;
	sprintf(file_name,"connectableUpStream_F_matrix%d.txt",f);
	input = fopen(file_name,"r");
	for(int i=1; i<=n; i++) {
		c = 0;
		buffer_index = 0;
...
void output(int start, int end, int *neuroindex, int amount, char neuroListOri[22835][20]) {
	...
	emptylist = fopen("connectableUpStream_F_matrix_empty.txt", "r");
	...
	for(zone=start; zone<=end; zone++) {
		...
		for(n_index=0; n_index<amount; n_index++) {
			...
			for(i=0; i<22835; i++) {
				...
				for(int k=0; k<value; k++) fprintf(output, "%s %s %d\n", name[zone-1], neuroListOri[i], zone);
			}
			fclose(output);
		}
		free(matrix);
