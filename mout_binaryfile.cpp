...
int main() {
    ...
    for(int f=start; f<=end; f++) {
        ...
        sprintf(file_name,"H:\\n2n0_matrix_bf\\result_matrix%d.txt",f);
        outtable = fopen(file_name,"wb");
        /*sprintf(file_name,"H:\\n2n0_matrix_bf\\range_counter%d.txt",f);
        rangetable = fopen(file_name,"w");*/
        counter = 0;
        printf("array ok\n");
        for(int ff=0; ff<4; ff++) {
            sprintf(file_name,"outtable%d.%d.txt",f,ff);
            ...
            while(true) {
                ...
                if(progress == 2) {
                    x=index[0]+1;
                    y=index[1]+1;
                    array[x+(y-1)*22835] = value;
                    array[y+(x-1)*22835] = value;
                    progress = 0;
                    counter++;
        ...
        for(int i=1; i<=n; i++) {
            ...
            sprintf(iobuffer, "%.12lf" ,array[i]);
            fwrite(iobuffer, 14, 1, outtable);
            //fprintf(outtable,"%.12lf ",array[i]);
            //if(i%22835 == 0) fwrite("\n", 2, 1, outtable);
            //if(i%22835 == 0) fprintf(outtable,"\n");
            }
        fclose(outtable);
