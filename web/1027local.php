			...
			echo "cytoscape({";
				...
				echo "style: [";
					...
					echo "css: {'width':'3.0', 'line-color': '#D3D3D3', 'content': 'data(relationship)'}},";
						...
						$downStream = $_GET['downStream'];
						if(isset($_GET["select_zone"]))$select_zone = $_GET["select_zone"];
						else $select_zone[0]=60;
					    $nodelist = fopen("nodelist.txt", "r");
						
						$file_nameDown = "downStream/".$_GET['neuron'];
						$file_nameUp = "upStream/".$_GET['neuron'];
						$inputDown = fopen($file_nameDown, "r");
						$inputUp = fopen($file_nameUp, "r");
						$totalEdge = 0;
						$totalNode = 0;
						$counter = 0;
						$skip = 0;
						...
						if( $select_zone[0] != 59 ) {
							if($downStream) {
								while(($buffer = fgets($inputDown, 50)) != false) {
									...
									$network[$totalEdge]['direction'] = "down";
									$totalEdge++;
								}
							}
							if($upStream) {
								while(($buffer = fgets($inputUp, 50)) != false) {
									sscanf($buffer, "%s %s %d\n", $nodeA, $nodeB, $interaction);
									if( $interaction == 59 ) break;
									if( $skip == $interaction ) continue;
									do {
									...
									for($i=0; $i<22893; $i++) {
										if( $nodeA == $node[$i] ) {
											$list[$i] = 1;
											$z++;
											$network[$totalEdge]['nodeBid'] = $i; //upstream 反向
											if($z == 2) break;
										}
										if( $nodeB == $node[$i] ) {
											$list[$i] = 1;
											$z++;
											$network[$totalEdge]['nodeAid'] = $i; //upstream 反向
											if($z == 2) break;
										}
									}
									//upstream 反向
									...
									$network[$totalEdge]['interaction'] = $interaction;
									$network[$totalEdge]['direction'] = "up";
									$totalEdge++;
								}
							}
						}
